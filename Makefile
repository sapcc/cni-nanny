...
generate: install-controller-gen
	...
	@controller-gen crd rbac:roleName=cni-nanny webhook paths="./..." output:crd:artifacts:config=crd output:rbac:artifacts:config=config/rbac
...
build/cover.out: FORCE generate install-setup-envtest | build
	...
	KUBEBUILDER_ASSETS=$$(setup-envtest use 1.32 -p path) go run github.com/onsi/ginkgo/v2/ginkgo run --randomize-all -output-dir=build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTFLAGS) $(GO_TESTPKGS)
